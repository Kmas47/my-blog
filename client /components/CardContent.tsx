...
export const CardContent = (props) => {
  const { title, description, createdAt, updatedAt } = props;
  ...
  return (
    <div>
      ...
      <div className="flex justify-end pt-1">
          <p className="text-sm font-light leading-5 tracking-wide capitalize px-3 pt-1 pb-3">Updated At - {formattedUpdatedAt} <span>{time}</span></p>
      </div>
      <div className="flex flex-row justify-end px-2 pb-3">
          <div className="bg-blue-50 rounded-full w-auto px-2 py-1">
            <ModeCommentOutlinedIcon />  <span className=" align-bottom text-xs font-light leading-5 tracking-wide">2</span>
          </div>
      </div>
    </div>
  );
